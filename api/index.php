...
$isVercel = (bool) ($_ENV['VERCEL'] ?? getenv('VERCEL'));

if ($isVercel || ($_ENV['APP_ENV'] ?? getenv('APP_ENV')) === false) {
    $defaults = [
        ...
        'DB_DATABASE' => '/tmp/perfume-database.sqlite',
    ...
    foreach ($defaults as $key => $value) {
        if ($isVercel || ($_ENV[$key] ?? getenv($key)) === false) {
            $_ENV[$key] = $value;
            $_SERVER[$key] = $value;
            putenv($key.'='.$value);
        }
...
$runtimeDatabase = '/tmp/perfume-database.sqlite';

if (is_file($bundledDatabase) && ! is_file($runtimeDatabase)) {
